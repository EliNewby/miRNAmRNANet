...
def qstest(network, communities, qfunc, sfunc, cdalgorithm, num_of_rand_net = 500, num_of_thread = 4, alpha = 0.05):
    q = np.array([qfunc(network, x) for x in communities], dtype = float)    
    s = np.array([sfunc(network, x) for x in communities], dtype = float)
    C = len(communities)
    ...
    print("Got Random Data")
    q_tilde = np.array(q_tilde, dtype = float)    
    s_tilde = np.array(s_tilde, dtype = float)    
...
def draw_qs_samples(network, communities, qfunc, sfunc, cdalgorithm, num_of_rand_net):
    deg_1 = [x[1] for x in network.degree() if 'hsa' in x[0]]
    deg_2 = [x[1] for x in network.degree() if 'hsa' not in x[0]]
    q_rand = []
    s_rand = []
    for i in range(num_of_rand_net):
        if(num_of_rand_net < 100):
            print(str(100*i//num_of_rand_net)+"%")
        else:
            if(i%(num_of_rand_net//100*10) == 0):
                print(str(i*100//num_of_rand_net)+"%")
        networkr = nx.bipartite.configuration_model(deg_1,deg_2)
        numTests = 0
        while(not nx.is_connected(networkr) and numTests < 1000):
            networkr = nx.bipartite.configuration_model(deg_1,deg_2)
            numTests += 1
        if(numTests == 1000):
            LCC = sorted(list(nx.connected_components(networkr)),key=len,reverse=True)[0]
            while(len(LCC)/len(networkr) < 0.99):
                networkr = nx.bipartite.configuration_model(deg_1,deg_2)
                LCC = sorted(list(nx.connected_components(networkr)),key=len,reverse=True)[0]
            networkr = nx.subgraph(networkr,LCC)
